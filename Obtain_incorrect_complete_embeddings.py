...
for file_name in ['train','val','test']:
    ...
    # Load the data
    file_path = target_dictionary + file_name + "_incorrect_predictions.csv"
    ...
    output_path = target_dictionary + file_name + "_incorrect_sequence_embeddings.tsv"
    ...
    print(file_name + "_incorrect_sequence_embeddings.tsv " + f"have been saved to {output_path}")

# Concatenate the three matrices vertically into a single matrix:
train_data = pd.read_csv("data/sample_data/result/train_incorrect_sequence_embeddings.tsv",sep='\t')
test_data = pd.read_csv("data/sample_data/result/test_incorrect_sequence_embeddings.tsv", sep='\t')
val_data = pd.read_csv("data/sample_data/result/val_incorrect_sequence_embeddings.tsv", sep='\t')
merged_data = pd.concat([train_data, test_data, val_data], ignore_index=True)


# Divide the sequence into 1024 features:
# Load the file.
output_path = target_dictionary + "merged_incorrect_sequence_embeddings_1024.tsv"
data = merged_data
...
print(f"The conversion is complete, and the features have been saved to {output_path}")
