...
def split_forward_and_reverse(directory_path, output_directory):
    ...
    dev_files = sorted([f for f in files if f.startswith("dev")], key=lambda x: int(x.split(".")[0].split("_")[-1]))
    ...
    def merge_files(file_list, prefix):
        ...
        for file_name in file_list:
            ...
            file1 = output_directory + prefix +"_shuffle.tsv"
            file2 = combined_data
            
        df1 = pd.read_csv(file1, sep='\t').iloc[0:,1]
        print("df1:",df1)
        df2 = combined_data
        ...
        merged_df.columns = ['name'] + [f"feature_{i}" for i in range(1, 769)] + ['probabilities_1','probabilities_0','label']
        print(prefix + "_correct_predictions.csv：",merged_df)
         
        correct_predictions = merged_df[(merged_df['label'] == 1) & (merged_df['probabilities_1'] > 0.9) |
                                (merged_df['label'] == 0) & (merged_df['probabilities_0'] > 0.9)]
        incorrect_predictions = merged_df[(merged_df['label'] == 1) & (merged_df['probabilities_1'] < 0.1) | 
                                 (merged_df['label'] == 0) & (merged_df['probabilities_0'] < 0.1)]
    ...
    # Merge the train, dev, and test files.
    merge_files(train_files, "train")
    merge_files(dev_files, "dev")
